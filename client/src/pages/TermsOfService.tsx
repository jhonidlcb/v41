...
export default function TermsOfService() {
  ...
  const { data: pageData, isLoading } = useQuery({
    queryKey: ["/api/public/legal/terms"],
    queryFn: async () => {
      const response = await apiRequest("GET", "/api/public/legal/terms");
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-3xl font-bold text-center">
                {isLoading ? "Cargando..." : pageData?.title || "Términos y Condiciones de Servicio"}
              </CardTitle>
